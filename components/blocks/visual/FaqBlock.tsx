...
const AccordionVariant: React.FC<{ items: FAQBlockProps['content']['items']; blockColor: string; blockId: string; anim: any }> = ({ items, blockColor, blockId, anim }) => (
  <>
    <div className="w-full" style={{ maxWidth: 'var(--block-max-width)' }}>
      {items.map((item, index) => {
        const itemDelay = anim.delay + 0.1 + (index * 0.05);
        return (
          <details
            key={index}
            ...
            style={{ 
              ...
              '--siti-anim-duration': anim.duration + 's',
              '--siti-anim-delay': itemDelay + 's'
            } as any}
            data-siti-anim={anim.type}
            data-siti-anim-duration={anim.duration}
...
const ClassicVariant: React.FC<{ items: FAQBlockProps['content']['items']; blockColor: string; blockId: string; anim: any }> = ({ items, blockColor, blockId, anim }) => (
  <>
    <div
...
const SideBySideVariant: React.FC<{ items: FAQBlockProps['content']['items']; blockColor: string; viewport?: string; anim: any }> = ({ items, blockColor, viewport, anim }) => {
...
const NumberedVariant: React.FC<{ items: FAQBlockProps['content']['items']; blockColor: string; anim: any }> = ({ items, blockColor, anim }) => (
  <div className="w-full" style={{ maxWidth: 'var(--block-max-width)' }}>
    {items.map((item, index) => {
      const itemDelay = anim.delay + 0.1 + (index * 0.05);
      return (
...
export const FAQBlock: React.FC<FAQBlockProps> = ({ content, block, project, viewport, isStatic, onInlineEdit }) => {
  ...
  return (
    <FAQWrapper content={content} block={block} style={style} project={project} isStatic={isStatic} onInlineEdit={onInlineEdit}>
      {variant === 'accordion' && <AccordionVariant items={items} blockColor={blockColor} blockId={block.id} anim={anim} />}
      {variant === 'classic' && <ClassicVariant items={items} blockColor={blockColor} blockId={block.id} anim={anim} />}
      {variant === 'side-by-side' && <SideBySideVariant items={items} blockColor={blockColor} viewport={viewport} anim={anim} />}
      {variant === 'numbered' && <NumberedVariant items={items} blockColor={blockColor} anim={anim} />}
    </FAQWrapper>
  );
};
