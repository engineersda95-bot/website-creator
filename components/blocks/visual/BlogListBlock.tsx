      ...
      >
        ...
        <div className="flex-1 flex flex-col">
          {post.categories?.length > 0 && (
            <span className="text-[10px] font-bold uppercase tracking-wider mb-1.5" style={{ color: tagColor }}>
          ...
          {(showAuthor || showDate) && (
            <div className="flex items-center gap-3 mt-auto" style={{ fontSize: '0.7rem', opacity: 0.35 }}>
                  ...
                  {isStatic
                    ? (post.authors || []).map((a, i) => (
                        <React.Fragment key={i}>
                          {i > 0 && ', '}
                          <a href={`${blogLangPrefix}/blog?author=${encodeURIComponent(a)}`}>{a}</a>
                        </React.Fragment>
                      ))
                    : (post.authors || []).join(', ')
                  }
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full mx-auto">
          ...
          return (
            <div className="mb-8 space-y-4">
              ...
              <style dangerouslySetInnerHTML={{ __html: `
                ...
                .siti-filter-pill.active { background: ${filterActiveBg || 'currentColor'}; color: ${filterActiveText || 'var(--block-bg, #fff)'}; border-color: ${filterActiveBg || 'currentColor'}; }
        ...
        {/* View all link */}
        {content.showViewAll && posts.length > 0 && (
          <div className={cn("mt-8", style.align === 'center' ? "text-center" : style.align === 'right' ? "text-right" : "text-left")}>
            <a href={`${blogLangPrefix}/blog`} className="inline-flex items-center gap-2 text-sm font-semibold transition-opacity hover:opacity-70" style={{ color: pColor }}>
