...
export const ConfigSidebar: React.FC = () => {
  ...
  if (!selectedBlock) {
    return (
      <div className="w-80 bg-white border-l border-zinc-200 flex flex-col h-full shadow-sm animate-in slide-in-from-right duration-500 overflow-y-auto">
         ...
         <div className="p-6 space-y-10">
            ...
            <section className="pt-8 border-t border-zinc-100 pb-24">
               <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-6 flex items-center gap-2">
                 <Zap size={14} className="text-amber-500" /> Funzioni Extra
               </h3>
               <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 bg-zinc-50 rounded-2xl border border-zinc-100 shadow-sm hover:border-zinc-200 transition-all">
                     <div>
                       <label className="text-[10px] font-black text-zinc-900 uppercase block">CTA Fluttuante</label>
                       <p className="text-[8px] text-zinc-400 font-bold uppercase tracking-wider">Pulsante rapido (es: WhatsApp/Tel)</p>
                     </div>
  ...
  return (
    <div className="w-80 bg-white border-l border-zinc-200 flex flex-col h-full shadow-sm animate-in slide-in-from-right duration-200">
            ...
            {selectedBlock.type === 'navigation' && (
              <div className="space-y-6">
                 ...
                 <div className="pt-4 border-t">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                       <button onClick={() => updateContent({ layoutType: 'standard' })} className={cn("py-2.5 text-[10px] font-bold border-2 rounded-xl transition-all", (selectedBlock.content.layoutType || 'standard') === 'standard' ? "bg-zinc-900 text-white border-zinc-900" : "text-zinc-400 border-zinc-100")}>LISTA (Desktop)</button>
                       <button onClick={() => updateContent({ layoutType: 'hamburger' })} className={cn("py-2.5 text-[10px] font-bold border-2 rounded-xl transition-all", selectedBlock.content.layoutType === 'hamburger' ? "bg-zinc-900 text-white border-zinc-900" : "text-zinc-400 border-zinc-100")}>HAMBURGER (Sempre)</button>
                    </div>
                 </div>
              </div>
            )}
              ...
              <div className="space-y-6">
                  <div>
                    <label className="text-[10px] font-bold text-zinc-400 uppercase mb-3 block flex justify-between">
                      ...
                      <span className="text-zinc-900 font-bold">{getStyleValue(['navigation', 'text'].includes(selectedBlock.type) ? 'fontSize' : 'titleSize', selectedBlock.type === 'navigation' ? 14 : 40)}px</span>
                    </label>
                    <div className="flex gap-2">
                       <input 
                        ...
                        value={getStyleValue(['navigation', 'text'].includes(selectedBlock.type) ? 'fontSize' : 'titleSize', selectedBlock.type === 'navigation' ? 14 : 40)}
                        onChange={(e) => updateStyle(['navigation', 'text'].includes(selectedBlock.type) ? { fontSize: parseInt(e.target.value) } : { titleSize: parseInt(e.target.value) })}
                      />
