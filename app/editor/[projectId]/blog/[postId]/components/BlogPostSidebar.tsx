...
import { Save, Loader2, Settings, Search, Layout, X } from 'lucide-react';
...
interface BlogPostSidebarProps {
  ...
  onSetSidebarSection: (section: 'config' | 'seo' | 'style') => void;
  onCoverUpload: (base64: string) => Promise<void>;
  ...
  return (
    <div className="w-72 bg-white border-l border-zinc-200/80 flex flex-col shrink-0 overflow-hidden">
      <div className="flex border-b border-zinc-100 shrink-0">
        {([
          { id: 'config', icon: Settings, label: 'Dettagli' },
          { id: 'seo', icon: Search, label: 'SEO' },
          { id: 'style', icon: Layout, label: 'Stile' },
        ] as const).map(({ id, icon: Icon, label }) => (
          <button
            key={id}
            onClick={() => onSetSidebarSection(id)}
            className={cn("flex-1 py-2.5 text-[11px] font-bold transition-all border-b-2 -mb-px flex items-center justify-center gap-1.5",
              sidebarSection === id ? "border-zinc-900 text-zinc-900" : "border-transparent text-zinc-400"
            )}
          >
            <Icon size={12} /> {label}
          </button>
