...
import { CheckCircle2, Circle, Bell, ChevronDown } from 'lucide-react';
...
export function ChecklistTab({ project, pages, siteGlobals, onOpenSeo, onOpenSettings }: ChecklistTabProps) {
  ...
  const avgPageScore = pageScoresData.length > 0
    ? Math.round(pageScoresData.reduce((s, p) => s + p.score, 0) / pageScoresData.length)
    : 0;
