import 'server-only';
import { BlogPost, Project, Page, ProjectSettings } from '@/types/editor';
        ...
        <a href="${langPrefix}/blog/${post.slug}" class="block group">
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center gap-3 text-xs" style="opacity: 0.4;">
              ${(post.authors || []).length > 0 ? `<span>${(post.authors || []).map(a => `<a href="${langPrefix}/blog?author=${encodeURIComponent(a)}" style="text-decoration:none;color:inherit;">${a}</a>`).join(', ')}</span><span>&middot;</span>` : ''}
              <span>${date}</span>
            </div>
          </div>
        </a>
...
<head>
  ...
  <style>
    :root { --primary: ${pColor}; --font-main: '${font}', sans-serif; }
    * { font-family: inherit; margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: var(--font-main); background: ${themeBg}; color: ${themeText}; }
...
<body>
  <div style="max-width: 1100px; margin: 0 auto; padding: 60px 24px;">
    <div style="margin-bottom: 40px;">
      <a href="/" style="font-size: 12px; font-weight: 600; opacity: 0.4; margin-bottom: 12px; display: inline-block;">&larr; Torna al sito</a>
      <h1 style="font-size: 2.5rem; font-weight: 800; letter-spacing: -0.03em; line-height: 1.1;">Blog</h1>
...
export function generateBlogPostHtml(
  ...
  // Render nav and footer from project pages
  let navHtml = '';
  let footerHtml = '';
  if (allPages.length > 0) {
    const { renderBlock } = require('./generate-static');
    // Find nav and footer blocks from any page
    let navBlock = null as any;
    let footerBlock = null as any;
    for (const page of allPages) {
      for (const b of (page.blocks || [])) {
        if (!navBlock && b.type === 'navigation') navBlock = b;
        if (!footerBlock && b.type === 'footer') footerBlock = b;
        if (navBlock && footerBlock) break;
      }
      if (navBlock && footerBlock) break;
    }
    if (navBlock) navHtml = renderBlock(navBlock, allPages, project, renderToStaticMarkup);
    if (footerBlock) footerHtml = renderBlock(footerBlock, allPages, project, renderToStaticMarkup);
  }
  ...
  const bodyHtml = post.blocks.map(block => {
    if (block.type === 'text' && block.content?.text) {
      ...
      return `<div class="blog-body rt-content" style="font-size: 1.1rem; line-height: 1.8; max-width: 100%;">${rendered}</div>`;
    }
    return '';
  }).join('\n');
...
<head>
  ...
  <style>
    ...
    * { font-family: inherit; margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .blog-body h2 { font-size: 1.5rem; font-weight: 700; margin: 2em 0 0.5em; }
    .blog-body h3 { font-size: 1.2rem; font-weight: 600; margin: 1.5em 0 0.5em; }
...
<body>
  ...
  <article style="max-width: 720px; margin: 0 auto; padding: 48px 24px;">
    ...
    ${(post.categories || []).length > 0 ? `<div style="margin-bottom: 12px;"><span style="font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; color: ${pColor};">${post.categories.join(' · ')}</span></div>` : ''}

    <h1 style="font-size: 2.5rem; font-weight: 800; letter-spacing: -0.03em; line-height: 1.1; margin-bottom: 16px;">${post.title}</h1>

    ${post.excerpt ? `<p style="font-size: 1.15rem; opacity: 0.5; line-height: 1.6; margin-bottom: 24px;">${post.excerpt}</p>` : ''}

    <div style="display: flex; align-items: center; gap: 12px; font-size: 13px; opacity: 0.4; margin-bottom: 48px; padding-bottom: 24px; border-bottom: 1px solid color-mix(in srgb, ${themeText} 8%, transparent);">
      ${(post.authors || []).length > 0 ? `<span style="font-weight: 600;">${(post.authors || []).map(a => `<a href="${langPrefix}/blog?author=${encodeURIComponent(a)}" style="text-decoration:none;color:inherit;">${a}</a>`).join(', ')}</span><span>&middot;</span>` : ''}
      <span>${date}</span>
    </div>
...
<body>
  <div style="max-width: 1100px; margin: 0 auto; padding: 60px 24px;">
    <div style="margin-bottom: 40px;">
      <a href="/blog" style="font-size: 12px; font-weight: 600; opacity: 0.4; margin-bottom: 12px; display: inline-block;">&larr; Torna al blog</a>
      <h1 style="font-size: 2.5rem; font-weight: 800; letter-spacing: -0.03em; line-height: 1.1;">Articoli di ${author}</h1>
